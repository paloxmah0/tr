...
impl Db {
    pub async fn create_note(&self, account_id: Uuid, req: &CreateNote) -> AppResult<Note> {
        let row = sqlx::query(
            r#"INSERT INTO notes (account_id, title, content, content_type, status)
    ...
    pub async fn mark_note_extracted(&self, id: Uuid) -> AppResult<()> {
        sqlx::query("UPDATE notes SET status = 'extracted', processed_at = now(), error = NULL WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    pub async fn mark_note_failed(&self, id: Uuid, err: &str) -> AppResult<()> {
        sqlx::query("UPDATE notes SET status = 'failed', processed_at = now(), error = $2 WHERE id = $1")
            .bind(id)
            .bind(err)
            .execute(&self.pool)
            .await?;
