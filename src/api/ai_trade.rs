...
pub async fn place_trade(
    ...
    if account.mode == TradingMode::Live {
        let broker = state.markets.broker_for(class)
            .ok_or_else(|| AppError::Execution("No broker configured for this asset class. Add your API token in Settings.".into()))?;
        ...
        let order = broker.place_order(order_req).await?;
